...
class Minesweeper{
    ...
    void displayGrid(){
        ...
        for(int i = 0 ; i < size ; i++){
            ...
            for(int j = 0 ; j < size ; j++){ 
                ...
                //display cell
                if (grid[i][j].isHidden == false){  //show grid value or -- if cell is still hidden  
                    cout << grid[i][j].value << " ";
                } else {
                    cout << "-- ";
                }
    ...
    bool checkMine(int y, int x){ //to check if cell is a mine
    ...
    void incrementValue(int y, int x){ //to change string to int, increment and then back to string 
        int tempInt = 0;
    ...
    void generateGridValues(){ //to generate grid values at the start of the game
    ...
    void userInput(){
        ...
        while(true){
            ...
            if(grid[userY][userX].isHidden == false){ //to check if cell is already showing
    ...
    void showCell(int y, int x){ //show cell

        if(grid[y][x].isHidden == true){    //in case cell is already showing, so cellsOpened does not get incremented when it shouldnt  
    ...
    void checkConditions(int x, int y){
        ...
        if(grid[y][x].value == "XX"){ //mine is hit
